...
const closeSheet = ref(false);
watch(route, () => {
  closeSheet.value = false;
});
  ...
  <Sheet v-model:open="closeSheet" class="transition-all duration-300">
    <SheetTrigger as-child class="">
      <Button
        variant="outline"
        size="icon"
        class="dark:bg-transparent outline-0 border-none rounded-none"
    ...
    <SheetContent
      ...
      force-mount
      as-child
      class="bg-indigo-950/80 border-l border-indigo-500/20 z-200"
    >
      <Motion
        key="sheet-content"
        :initial="{ x: '100%' }"
        ...
        <section class="p-4 text-indigo-300 text-[20px] h-full flex flex-col gap-5">
          <div class="flex flex-col justify-between h-auto">
            <NuxtLink
              ...
              :to="item.to"
              class="text-white hover:bg-indigo-100/20 px-2 p-1 transition-all duration-300"
              >{{ item.label }}</NuxtLink
            >
          </div>
          <div class="flex gap-2.5 items-center text-base">
            <NuxtLink
              to="/documents/Suleiman-Hamza-2026-Resume.pdf"
              external
              download="Suleiman Hamza Resume 2026.pdf"
              class="cursor-pointer flex items-center gap-2 bg-slate-800 text-gray-200 p-2 px-3 w-fit rounded-lg"
            >
              <Download :size="16" />
              Resume
            </NuxtLink>
            <NuxtLink
              ...
              external
              target="_blank"
              class="cursor-pointer flex items-center gap-1 bg-slate-800 text-gray-200 p-2 w-fit rounded-lg"
            >
              View Online
